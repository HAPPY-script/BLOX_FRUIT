return function(sections)
    ...
    do
        ...
        local function normalizeText(txt)
            ...
            local s = tostring(txt):gsub("<[^>]+>", "")         -- remove RichText tags
        ...
        local function tryProcess()
            ...
            if not execFrame or not execFrame:IsA("ScrollingFrame") then
                return
            end

            local validSGs = scanValidScreenGuis(pg)
            if #validSGs < MIN_SG_REQUIRED then
                return
            end
